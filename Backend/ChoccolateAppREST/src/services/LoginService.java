...
public class LoginService {
	...
	@OPTIONS
    @Path("/getCustomerPoints/{userId}")
    @Produces(MediaType.APPLICATION_JSON)
    public boolean corsGetCustomerPoints() {
        return true;
    }
	...
	@Path("/getCustomerPoints/{userId}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getCustomerPoints(@PathParam("userId") int id)
	{
		UserDAO dao = (UserDAO) ctx.getAttribute("userDAO");
		...
		if(customer==null)
		{
			return Response.status(405).entity("there is no customer with this id").build();
		}
		
		return Response.status(200).entity(customer.getPoints()).build();	
	}
