<template>
  ...
  <div class="card-body" >
    ...
    <p class="card-text">Please enter manager details below</p>
...
const newUser = ref({factoryId:0,username:"",password:"",name:"",surname:"",gender:"",birthday:"",role:"MANAGER"});

function registerNewUser(event) {
	event.preventDefault();
    axios.post("http://localhost:8080/ChoccolateAppREST/rest/registerManager", this.newUser, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('jsonWebToken')}`
      }
      
    }).then(response => {
        router.push('/allUsersView')
    }).catch(error => {
        console.log(error);
    });
}
