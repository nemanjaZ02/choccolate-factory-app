...
{
    axios.get(`http://localhost:8080/ChoccolateAppREST/rest/ChocolateFactoryService/getFactoryById/`+factoryId)
    .then(response=>{
...
function sortPurchases() {
    if (sortOption.value === "date_asc") {
        filteredPurchases.value.sort((a, b) => {
            const dateA = new Date(formatDate2(a.dateAndTime));
            const dateB = new Date(formatDate2(b.dateAndTime));
            return dateA - dateB;
        });
...
function search() {
    filteredPurchases.value = purchases.value.filter(purchase => 
        ...
        (minimumDate.value == "" || minimumDate.value == 0 || (new Date(formatDate2(purchase.dateAndTime)) >= new Date(convertAndFormatDate(minimumDate.value)))) &&
        (maximumDate.value == "" || maximumDate.value == 0 || (new Date(formatDate2(purchase.dateAndTime)) <= new Date(convertAndFormatDate(maximumDate.value))))
    );
