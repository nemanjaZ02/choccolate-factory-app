<template>
    <div v-if="dataLoaded && !purchase.isDeleted" style="margin-top: 100px;">
        ...
        <div class="row">
         ...
         <div style="display: flex; flex-direction: column; align-items: flex-start;" v-if="purchase.state == 'Processing'">
         <button v-if="loggedInUser.role=='MANAGER' && purchase.state == 'Processing'" type="button" class="btn btn-success" style="margin-bottom: 10px; height: 50px; margin-left: 50px;" @click="updatePurchaseStatus('Accepted')">Accept</button>
         <button v-if="loggedInUser.role=='CUSTOMER' && purchase.state == 'Processing'" type="button" class="btn btn-danger" style="margin-bottom: 10px; height: 50px; margin-left: 50px;" @click="updatePurchaseStatus('Canceled')">Cancel</button>
         <label v-if="loggedInUser.role=='CUSTOMER' && purchase.state == 'Processing'" style="color: red; margin-left: 50px;" >Warning: Canceling the order results in losing points</label>
        <div v-if="purchase.state == 'Processing'" style="display: flex; flex-direction: row; align-items: center; margin-left: 50px;">
